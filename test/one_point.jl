...
import BootstrapVirasoro: LRPosCache, qfromτ, xfromq, qfromx,
    eval_series
...
@testset "Fder" begin
        ...
        function eval_series_der(τ)
                ...
                byhand = (BootstrapVirasoro.eval_series(b_p, τ) - 
                BootstrapVirasoro.eval_series(b_m, τ)) / (2 * ϵ)
                series_der - byhand
        end
        ...
        function eval_block_der(τ)
                ...
                byhand = (BootstrapVirasoro.eval(b_p, τ) -
                 BootstrapVirasoro.eval(b_m, τ)) / (2 * ϵ)
                block_der - byhand
        end
...
@testset "Freg" begin
        ...
        @test isapprox(
                bϵ.cblocks.left(τ),
                co.Rmn.left[V0.r, V0.s] / ϵ *
                BootstrapVirasoro.eval_lr_op(b, lr_cache)[1] + b.cblocks.left(τ),
                rtol=1e-40,
...
@testset "Interchiral" begin
        ...
        prefactor =
                BootstrapVirasoro.PosCache(2τ, b.blocks[1][:left]).prefactor *
        ...
        for i = 1:3
                @test isapprox(
                        b.blocks[i](2τ) / prefactor,
                        b_s.blocks[i](x) / prefactor_s *
        ...
        @test isapprox(
                (eval(b.blocks[1], 2τ) + eval(b.blocks[2], 2τ) * s) / prefactor,
                (eval(b_s.blocks[1], x) + eval(b_s.blocks[2], x) * s_s) /
                prefactor_s * 16^(-4P^2),
                rtol=1e-20,
        )
        ...
        @test isapprox(
                eval(b, 2τ) / prefactor,
                eval(b_s, x) / prefactor_s * 16^(-4P^2),
                rtol=1e-20,
        )
end
