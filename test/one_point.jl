...
@testset "Freg" begin
    ...
    @test isapprox(
        evaluate(bϵ[:left], 2τ),
        co._Rmn[:left][:s][V0.r, V0.s] / ϵ * BootstrapVirasoro.evaluate_lr_op(b, lr_cache)[:left] + evaluate(b[:left], 2τ),
        rtol=1e-40
...
pref_s = total_prefactor(b_s[:left], x) * total_prefactor(b_s[:right], conj_q(x, b_s.corr))
pref2 = total_prefactor(b2[:left], 2τ) * total_prefactor(b2[:right], conj_q(2τ, b.corr))
pref2_s = total_prefactor(b2_s[:left], x) * total_prefactor(b2_s[:right], conj_q(x, b_s.corr))
...
@testset "Interchiral" begin
    ...
    prefactor = BootstrapVirasoro.PositionCache(2τ, b.blocks[1][:left]).prefactor *
        BootstrapVirasoro.PositionCache(conj_q(2τ, b.blocks[1].corr), b.blocks[1][:right]).prefactor

    b_s = Block(co_s, :s, V_s, interchiral=true, Δmax=10)
    prefactor_s = BootstrapVirasoro.PositionCache(x, b_s.blocks[1][:left]).prefactor *
        BootstrapVirasoro.PositionCache(conj_q(x, b_s.blocks[1].corr), b_s.blocks[1][:right]).prefactor
