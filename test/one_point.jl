...
@testset "Fder" begin
    ...
    @test abs(eval_series_der(big"10" + big"0.01" * im)) < big"1e-16"
    ...
    eta = etaDedekind(Acb(τ))
    @test b(τ) / eval_series(b, τ) ≈ q^(V0.dims[:left].δ) / eta
    ...
    for val in test_vals
        @test abs(eval_block_der(val)) < big"1e-7"
    end
end
...
@testset "ell" begin
    ...
    for (i, b) in enumerate(bs)
        @test isapprox(ell(b) / sqrt(big"2") + 16 * log(big"2") / c_S2.β * Vs[i].s,
                       ell(bs_S2[i]), rtol=1e-25)
    end
end
...
@testset "Full log block" begin
    for (i, b) in enumerate(bs)
        @test isapprox(b(τ) / prefs[i], bs_S2[i](x) / prefs_S2[i], rtol=1e-25)
    end
end
...
@testset "Sphere <-> torus prefactors" begin
    ...
    @test isapprox(prefA(τ, co_S2, :s, :left),
    ...
    @test isapprox(prefA(-conj(τ), co_S2, :s, :right),
    ...
    @test isapprox(ncprefA(τ, co_S2, :s),
    ...
    for ind in [
        ...
        for chan in BootstrapVirasoro.CHANNELS
            @test isapprox(b(ts[chan]) * 16^(2P^2 + 2Pbar^2),
