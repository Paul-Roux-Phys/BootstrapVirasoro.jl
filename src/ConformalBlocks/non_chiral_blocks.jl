...
function FactorizedBlock(co::Correlation{T,U}, chan, V, Nmax) where {T,U}
    ...
    FactorizedBlock{T,W}(V, co, LeftRight((left, right)), Nmax)
end
...
function LogarithmicBlock(co::CorrelationNonChiral{T,U}, chan, V, Nmax) where {T,U}
    ...
    left, left_op, right, right_op = Tuple(
        ChiralBlock(co, chan, v, lr, Nmax) for lr in (:left, :right) for v in VV
    )
...
reflect(b::NonChiralBlock) =
    FactorizedBlock(b.corr, b.channel, reflect(b.channel_field), b.Nmax)
...
function ell(V::FourFields, chan, r, s)
    ...
    βsq = β^2
    res = 4 * (π / tan(s * (π / β^2)))
    res += 4 * sum(digamma_reg(-r + j/βsq) + digamma_reg(r + j/βsq) for j = (1-s):s)
    res -= sum(
        digamma_reg(
            1 // 2 +
            (lr == :left ? -1 : 1) *
    ...
    res -=
        2 * sum(
            digamma_reg(
                1//2 +
                pm1 * inv(β)^2 / 2 +
                (pm2 * V[1].dims[lr].P + 2 * (lr == :left ? -1 : 1) * P_rs(r, j, β)) /
                β,
